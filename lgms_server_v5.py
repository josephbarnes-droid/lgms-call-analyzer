"""
Little Guys Movers - Call Analyzer Server v5
=============================================
...
- If the transcript has fewer than 80 words OR is clearly not a sales estimate call, set call_type accordingly and still complete the rest of the analysis but note it should be excluded from scoring
...
- Return the rep's first name or full name if found, else return "Unknown"
...
KEYWORDS — check if rep said these (mark true/false):
...
For too_short or non_sales calls, score everything 0 and note why.
...
def read_html():
    for name in ["lgms_dashboard.html"]:
...
def supa(method, path, body=None):
    if not SUPABASE_URL or not SUPABASE_KEY:
        raise Exception("Supabase not configured — set SUPABASE_URL and SUPABASE_KEY env vars")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        routes = {"/analyze": self._analyze, "/save": self._save,
                  "/update": self._update, "/delete": self._delete, "/bulk_delete": self._bulk_delete}
...
if __name__ == "__main__":
    print("="*55)
    print("  Little Guys Movers — Call Analyzer Server v5")
    print("="*55)
    missing = [v for v in ["ANTHROPIC_API_KEY","SUPABASE_URL","SUPABASE_KEY"] if not os.environ.get(v)]
    if missing:
